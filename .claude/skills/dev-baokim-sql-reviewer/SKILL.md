...
description: |
  Skill review SQL query và DDL theo standards Baokim. Apply rules SQR (Query rules), IDR (Index/Partition), BKM02/04/07 (Eloquent only, no query in loop, hash search via DB index). Đầu vào: SQL query (raw hoặc Eloquent), DDL CREATE TABLE/ALTER, hoặc Laravel Repository code có query. Đầu ra: violations report với rule ID (SQR01.1, BKM04...) + severity (M/R) + fix proposal kèm code refactor + EXPLAIN suggestion để verify performance. Cover: SELECT * detection, LIMIT OFFSET vs seek pagination, LIKE '%...' index killer, IN params > 500, transaction size limits, redundant indexes, missing partition cho log tables, N+1 query patterns, hash search trong PHP code.
  Triggers: "review SQL query này", "check SQL có vi phạm SQR không", "review DDL CREATE TABLE", "review Repository code có query", "performance review query", "EXPLAIN analyze SQL", "kiểm tra index có dư thừa không", "tối ưu pagination N+1".
  Anti-triggers: schema design from scratch (dùng `dev-baokim-db-schema-designer`), code architecture review không liên quan query (dùng `dev-baokim-laravel-repo-pattern`), NoSQL queries (MongoDB/Redis), pure database administration (backup, replication).
owner: duy@baokim.vn
version: 0.1.0
lifecycle: draft
domain: dev
