...
name: vn-pii-masker
description: |
  Cross-domain skill mask PII Vietnamese trong logs, audit trails, error messages, transcripts, exports. Patterns hỗ trợ: CCCD/CMND (12 hoặc 9 số), Passport (1 chữ cái + 7 số), MST doanh nghiệp (10/13 số), phone VN (10 số 03/05/07/08/09), email, full name VN (mask middle name), account number ngân hàng (10-14 số), IP address (v4/v6), date of birth, address. Đầu vào: text/JSON/log line có thể chứa PII. Đầu ra: text/JSON đã mask + audit trail (PII types detected + count + masking pattern applied). KHÔNG mask trong response trả end user — chỉ mask trong log/audit/export.
  Triggers: "mask PII trong log", "che thông tin cá nhân", "ẩn CCCD trong audit", "anonymize log", "redact PII", "mask email trong export", "che số điện thoại trong error message", "PII masking cho compliance".
  Anti-triggers (KHÔNG dùng skill này khi): cần strong anonymization cho data analysis/ML training (skill này chỉ display masking, không cryptographic), mask binary data (cần specialized tool), mask trong response trả end user (KSNB cần đọc raw để nhập liệu), mask file ảnh/PDF (skill này text-only — dùng image redaction tool riêng).
owner: duy@baokim.vn
version: 0.1.0
lifecycle: draft
domain: cross
