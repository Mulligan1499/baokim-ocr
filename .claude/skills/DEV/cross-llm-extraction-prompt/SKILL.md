...
description: Foundation skill thiết kế prompt LLM extraction task (text/vision input). Encode 7 anti-hallucination patterns + JSON schema + few-shot. Cross-domain reusable cho OCR, CV parsing, contract extraction, feedback triage. Specialized skills inherit từ skill này.
when_to_use: |
  Triggers: "thiết kế prompt extraction", "anti-hallucination prompt",
  "structured output prompt LLM", "prompt cho LLM extract data từ", "prompt extract
  structured data", "extraction prompt template", "prompt cho [task] cần JSON output".

  Anti-triggers (KHÔNG dùng skill này khi):
  - Conversational chatbot prompt (không phải extraction)
  - Code generation prompt → dùng `dev-*` skills
  - Pure translation prompt (không có structured extraction)
  - Image generation prompt (DALL-E/MJ)
  - Prompt review/debug → dùng `prompt-reviewer` (chưa build)
...
### Example 4: Cross-domain negative anchor (HR CV parsing — không phải OCR)

**Bối cảnh**: Team HR muốn dùng AI parse CV ứng viên, extract email + phone + experience. Negative example dưới đây show AH-1 (empty thay vì guess) **hoạt động ngoài OCR domain**.

**Extraction contract**:
- Input type: text (CV plain text extracted từ PDF/Word)
- Critical fields: full_name, email, phone, total_years_experience
- Refusal: text không phải CV (vd: cover letter only, generic article)

**Negative few-shot trong prompt** (anchor refusal cho text-based):

```
INPUT (negative case 1 — CV blurry email):
"Nguyễn Văn A
Email: nguyenva**@gmail.c**
Phone: 0901-***-567
Experience: 3 years at FPT (2021-2024)..."
...
EXPECTED OUTPUT:
{
  ...
  "total_years_experience_confidence": 0.9
}

EXPLANATION: Email và phone bị che (** = mất ký tự), KHÔNG được đoán hoàn chỉnh. Trả empty + confidence=0. Years experience đọc rõ "3 years" → keep với confidence cao.

---

INPUT (negative case 2 — không phải CV):
"Chào Sếp, em xin báo cáo tiến độ tuần này:
- Hoàn thành module thanh toán
- Đang fix bug login..."

EXPECTED OUTPUT:
{
  "document_detected": false,
  "reason": "Đây là báo cáo công việc nội bộ (work update), không phải CV ứng viên."
}
```

**Lý do quan trọng**: negative example **trong domain CV** thay vì OCR — chứng minh patterns AH portable. Cùng pattern AH-1 (empty thay vì guess) + AH-3 (refusal non-target-document) hoạt động cho:
- OCR: CCCD mờ → `value=""`
- CV parsing: email bị che → `email=""`
- Customer feedback: text không phải feedback → `document_detected=false`

→ Pattern là **invariant**, chỉ thay context. Đây là proof reusability cross-domain.
