...
description: Thiết kế prompt Stage 1 Document Classifier — Claude Haiku (rẻ/nhanh) classify doc type + language + extraction strategy hint. Taxonomy default 15 KSNB doc types + 1 fallback. Output prompt + JSON schema + 4 few-shot + PHP service skeleton.
when_to_use: |
  Triggers: "Stage 1 classifier prompt", "document type classification prompt",
  "phân loại tài liệu OCR", "Haiku prompt cho classify",
  "design Stage1ClassifierService", "language detection prompt",
  "extraction strategy hint".

  Anti-triggers (KHÔNG dùng skill này khi):
  - Full extraction prompt → dùng `dev-vision-prompt-designer` (Stage 2)
  - Classifier cho non-document (audio/video)
  - Generic ML classification (không phải LLM)
  - Classify với mô hình tự train (Tesseract + custom model)
...
**Reference**: file 02 Section 4 (Stage 1), Section 5 (15 doc types)
...
1. **Taxonomy**: list các class enum + 1 fallback `other`/`unknown`
   - Default cho project: 15 KSNB doc types từ file 02 Section 5
...
❌ KHÔNG hardcode 15 KSNB taxonomy inline trong prompt — pass qua variable để adapt cho domain khác
