...
description: |
  Skill phân tích implicit feedback của KSNB (action trên UI OCR) → identify patterns "AI extract sai cùng field nhiều lần" → đề xuất diff cho skill prompt liên quan, dạng markdown patch + reasoning. Đầu vào: aggregate `ocr_user_actions` table theo (doc_type, field_key, action_type) + duration_ms percentiles + edited_count. Đầu ra: JSON array {pattern_id, observation, hypothesis, target_skill_file, target_runtime_file, proposed_diff (unified diff text), confidence, suggested_action}. Skill KHÔNG tự commit/sửa file — chỉ đề xuất; engineer review qua GitHub PR rồi approve hoặc reject. Encode decision tree: pattern → target artifact (skill .md vs runtime prompt vs code regex vs config threshold). Reusability: dùng được cho mọi LLM extraction pipeline có implicit feedback (HR CV parsing, contract extraction, customer feedback triage...).
  Triggers: "analyze KSNB actions", "feedback to skill update", "propose skill diff from production data", "refine prompt from user actions", "ocr:analyze-actions command", "implicit feedback loop agent", "AI tự cải thiện từ KSNB workflow".
  Anti-triggers (KHÔNG dùng skill này khi): direct edit prompt file không qua review (đó là human decision), tạo skill mới hoàn toàn (human-only — skill này chỉ refine existing), explicit feedback form analysis (skill này chỉ implicit actions), training custom ML model (đây là prompt engineering, không phải ML).
owner: duy@baokim.vn
version: 0.1.0
lifecycle: draft
domain: cross
